...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        scores, cache_a1 = affine_relu_forward(X, self.params['W1'], self.params['b1'])
        scores, cache_z2 = affine_forward(scores, self.params['W2'], self.params['b2'])
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # print(loss)
        da1, grads['W2'], grads['b2'] = affine_backward(dscore, cache_z2)
        grads['W2'] += self.reg * self.params['W2']
        _, grads['W1'], grads['b1'] = affine_relu_backward(da1, cache_a1)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        reg_loss = self.reg * 0.5 * np.sum([(self.params['W' + str(l)] ** 2).sum() for l in range(1, self.num_layers + 1)])
        loss += reg_loss

        for l in range(self.num_layers, 0, -1):
            cache_cursor = -1
            if self.use_dropout and l != self.num_layers:
                dout = dropout_backward(dout, caches[l - 1][cache_cursor])
                cache_cursor -= 1
            if l < self.num_layers:
                dout, grads['W' + str(l)], grads['b' + str(l)] = affine_relu_backward(dout, caches[l - 1][cache_cursor])
            else:
                dout, grads['W' + str(l)], grads['b' + str(l)] = affine_backward(dout, caches[l - 1][cache_cursor])
            grads['W' + str(l)] += self.reg * self.params['W' + str(l)]
            cache_cursor -= 1
            if self.use_batchnorm and l != self.num_layers:
                dout, grads['gamma' + str(l)], grads['beta' + str(l)] = batchnorm_backward(dout, caches[l - 1][cache_cursor])
