...
trans_count = np.zeros((NUM_STATES, NUM_STATES, 2))
trans_prob = np.ones_like(trans_count) / NUM_STATES
reward_count = np.zeros((NUM_STATES, 2))
reward = np.zeros((NUM_STATES, ))
value = 0.1 * np.random.rand(NUM_STATES)
###### END YOUR CODE ######
...
while consecutive_no_learning_trials < NO_LEARNING_THRESHOLD:
    ...
    v0 = np.dot(trans_prob[state, :, 0], value)
    v1 = np.dot(trans_prob[state, :, 1], value)
    ...
    if new_state == NUM_STATES - 1:
        ...
        for a in range(2):
            for s in range(NUM_STATES):
                den = np.sum(trans_count[s, :, a])
                if den > 0:
                    trans_prob[s, :, a] = trans_count[s, :, a] / den
        for s in range(NUM_STATES):
            if reward_count[s, 1] > 0:
                reward_count[s, 0] /= reward_count[s, 1]
        ...
        while True:
            iterations += 1
            for s in range(NUM_STATES):
                v1 = np.dot(trans_prob[s, :, 0], value)
                v1 = np.dot(trans_prob[s, :, 1], value)
                new_value[s] = max(v0, v1)
